...
if(require("rsae", quietly = TRUE)) {
  ...
  res <- attr(predict(fitobject,newdata=newXandZ,variances = list(linPred=TRUE)),"predVar") ## sum of cols nu and beta in Table 7
...
p <- predict(hlfit, newdata=data.frame(x=unique(data.test$x)), variances=list(sumVar=TRUE))
...
expect_equal(attr(p, "sumVar")[1],c(`1`=0.918197),tolerance=1e-6)
## also tests that a single level of the factor in the newdata is not a problem
predict(hlfit, newdata=data.test[1,], variances=list(sumVar=TRUE))
...
p1 <- predict(hl,variances=list(sum=TRUE))
p2 <- predict(hl,newdata=ll[1,],variances=list(sum=TRUE))
expect_equal(attr(p1,"sumVar")[1],attr(p2,"sumVar")[1])
expect_equal(p1[1],p2[1])
# verif 'slice' mechanism inncluding Evar (hence new levels of ranef) 
lll <- ll
lll$idx <- lll$idx+1
lll$latitude <- lll$latitude+1
p4 <- predict(hl,newdata=lll[1:101,],variances=list(sum=TRUE))
p5 <- predict(hl,newdata=lll[101:102,],variances=list(sum=TRUE))
expect_equal(attr(p4,"sumVar")[101],attr(p5,"sumVar")[1])
expect_equal(p4[101],p5[1])
