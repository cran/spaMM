...
if (spaMM.getOption("example_maxtime")>6) {
  ...
  var(drop(simulate(modtp, newdata = new_data, nsim = 100000, type = "residual"))) ## OK ~1.02722
  var(drop(simulate(modtp, newdata = new_data, nsim = 100000, type = "marginal"))) ## OK ~1.02722+0.3396
  mean(simulate(modtp, newdata = new_data, nsim = 100000, type = "marginal")) ## OK ~Intercept =0.02889 as x is FALSE
  mean(simulate(modtp, newdata = new_data, nsim = 100000, type = "residual")) ## OK ~0.02889+ 0.2051344=ranef(mod)[[1L]][paste(d$id[1])]
}
