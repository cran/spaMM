# 'constructor' for sXaug_EigenSparse_QRP_scaled object
# from Xaug which already has a *scaled* ZAL 
def_sXaug_EigenSparse_QRP_scaled <- function(Xaug,weight_X,w.ranef,H_global_scale) {
  ...
  resu <- structure(Xaug,
                    sXaug_EigenSparse_QRP_scaled=create_sXaug_EigenSparse_QRP_scaled(),
                    get_from="get_from_MME.sXaug_EigenSparse_QRP_scaled",
                    w.ranef=w.ranef,
                    n_u_h=n_u_h,
...
create_sXaug_EigenSparse_QRP_scaled <- function() {
  ...
  locfn <- function(sXaug,which="",szAug=NULL,B=NULL) {
    if (is.null(BLOB$R_scaled)) {
      blob <- lmwith_sparse_QRp(sXaug,szAug,
                               ...
                               returnR=TRUE,pivot=TRUE) ## using RcppEigen; szAug may be NULL
      # sXaug = t(tQ) %*% R[,sP] but then also sXaug = t(tQ)[,sP'] %*% R[sP',sP] for any sP'
      BLOB$perm <<- blob$P + 1L 
      BLOB$R_scaled <<- blob$R
    ...
    if ( ! is.null(szAug)) {
      if (is.null(BLOB$t_Q_scaled)) {
        ...
        return(Matrix::solve(Matrix::crossprod(BLOB$R_scaled), crossprod(sXaug, szAug)[BLOB$perm])[BLOB$sortPerm,,drop=FALSE])
      } else {
    ...
    if ( is.null(BLOB$R_R_v)  && 
      ...
      Rblob <- lmwith_sparse_QRp(BLOB$R_scaled[,BLOB$sortPerm[ seq_len(attr(sXaug,"n_u_h")) ]],
                                 NULL,
                                 returntQ=TRUE, ## actually FALSE, cf lmwith_sparse_QRp code
                                 returnR=TRUE,pivot=TRUE)
      ## provides a QRP factorization of R_scaled[,sortPerm[seq_n_u_h]] so that 
      ## X[,seq_n_u_h]=Q Q_R_v R_R_v P_R_v
      BLOB$R_R_v <<- Rblob$R
      BLOB$perm_R_v <<- Rblob$P + 1L
    }
    ...
    if (which=="t_Q_scaled") {
      ...
      if (is.null(BLOB$hatval_Z_)) {
        ...
        if (is.null(BLOB$t_Qq_scaled)){
          BLOB$t_Qq_scaled <<- solve(t(BLOB$R_R_v), t( sXaug[, seq_len(attr(sXaug,"n_u_h"))[BLOB$perm_R_v] ] ) )
        } 
  ...
  environment(locfn) <- list2env(list(BLOB=list()),
                                 parent=environment(def_sXaug_EigenSparse_QRP_scaled))
  return(locfn)
} 

get_from_MME.sXaug_EigenSparse_QRP_scaled <- function(sXaug,which="",szAug=NULL,B=NULL,
                                            damping, LMrhs, ...) {
  resu <- switch(which,
                 "logdet_R_scaled_v" = {
                   absdiag_R_v <- attr(sXaug,"sXaug_EigenSparse_QRP_scaled")(sXaug,which="absdiag_R_v")
                   sum(log(absdiag_R_v))
                 },
                 "logdet_R_scaled_b_v" = {
                   R_scaled <- attr(sXaug,"sXaug_EigenSparse_QRP_scaled")(sXaug,which="R_scaled")
                 ...
                 "LevMar_step" = {
                   ...
                   R_scaled <- attr(sXaug,"sXaug_EigenSparse_QRP_scaled")(sXaug,which="R_scaled")
                   sortPerm <- attr(sXaug,"sXaug_EigenSparse_QRP_scaled")(sXaug,which="sortPerm")
                   diag_pRtRp <- attr(sXaug,"sXaug_EigenSparse_QRP_scaled")(sXaug,which="diag_pRtRp")
                 ...
                 ## all other cases:
                 attr(sXaug,"sXaug_EigenSparse_QRP_scaled")(sXaug,which=which,szAug=szAug,B=B)
  )
  return(resu)
}
