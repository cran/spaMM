...
auglinmodfit <- function(TT,ZAL,lambda_est,wranefblob,d2hdv2,qr.d2hdv2,w.resid,beta_eta,
  ...
  ## les deux arguments sont apparus lors d'une etape de restructuration du code: 
  ##ils deaient venir d'une version locale de auglinmodfit dans HLfit
  ## les autres variables venant directement de la frame HLfit
  ...
  for (innerj in 1:maxit.mean) {
    ...
    if (  !  .spaMM.data$options$USEEIGEN)  {
      if ( .spaMM.data$options$USE_SPARSE_Q && ( ! inherits(wAugX,"Matrix"))  ) wAugX <- Matrix(wAugX) ## cf ?Matrix for default wrt sparsity 
      SQR <- QRwrap(wAugX)
    } 
    old_betaV <- betaV
    ...
    if (HL[1]>0 && (! LMMbool )) { #pforpv>0 && removed since sscaled used for u_h estimation too...
      ...
      sscaled <- compute.sscaled(sqrt.ww=sqrt.ww,qr.d2hdv2=qr.d2hdv2) ## s detadmu
    } else sscaled <- 0L 
      ...
      if ( maxit.mean > 1 && LevenbergM) { ## default LevenbergM
        ...
        if (.spaMM.data$options$USEEIGEN) {
          if ( !is.null(control.HLfit$intervalInfo)) {
            ...
            betaVQ <- intervalBlob$betaVQ ; betaV <- intervalBlob$betaV
          } else {
  ...
  return(list(beta_eta=beta_eta,v_h=v_h,u_h=u_h,eta=eta,wranefblob=wranefblob,
              w.resid=w.resid,Sig=Sig,d2hdv2=d2hdv2,qr.d2hdv2=qr.d2hdv2,wAugX=wAugX,tXinvS=tXinvS,
