...
auglinmodfit <- function(TT,ZAL,lambda_est,wranefblob,d2hdv2,w.resid,beta_eta,
  ...
  `compute.sscaled` <- function(sqrt.ww,qr.d2hdv2) { ## needs qr.d2hdv2, ZAL, stop.on.error, d2hdv2, rWW, ZALI, family, dmudeta, BinomialDen, mu, eta... ETC
    ...
    if (any(is.na(c(vecdi1,vecdi2,vecdi3)))) { ## then we need to compute some of them
      ...
      Pdiag <- calc.Pdiag(ZAL=ZAL,c(w.resid,w.ranef),wAugZALI= wAugX[,-seq_len(pforpv)] )
      #
  ...
  for (innerj in 1:maxit.mean) {
    ...
    if (betaFirst)  { ### LeeL12 top p. 6 Appendix (code non optimise, useful for checking other algorithms) 
      ...
      tXinvS <- calc_tXinvS(Sig,X.pv,stop.on.error,lambda_est,ranFix) ## note dependence v_h -> eta -> Sig...
      ...
      if ( maxit.mean > 1 && LevenbergM) { ## default(for *G*LMM) LevenbergM
        ...
        conv_dbetaV <- bla$dbetaV## the one that will be used for assessing convergence, 
        ...
        if (.spaMM.data$options$USEEIGEN) {
          if ( !is.null(control.HLfit$intervalInfo)) {
            calcLikArglist <- list(mu=mu,u_h=u_h,dvdu=dvdu,lambda_est=lambda_est,phi_est=phi_est,
                               ...
                               processed=processed,family=family,prior.weights=prior.weights)
    ...
    if ( LevenbergM ) { ## for LMM, d2hdv2 is constant // mu
      if (pforpv>0) keep_Sig <- Sig
      keep_d2hdv2 <- d2hdv2
    }
    if (pforpv>0) {
      Sig <- Sigwrapper(ZAL,1/w.ranef,1/w.resid,ZALtZAL=ZALtZAL) ## update v_h -> blob$GLMweights -> w.resid -> Sig -> next beta estimate
    }
    d2hdv2 <- calcD2hDv2(ZAL,w.resid,w.ranef) ## update d2hdv2= - t(ZAL) %*% diag(w.resid) %*% ZAL - diag(w.ranef)
    ...
    if (LevenbergM) {     
      ...
      if (innerj==1L) { ## not a good idea to use the p_v computed for all u_h initially set to zero
        ...
        if (currentlik==-Inf) { ## obs in binary probit with extreme eta... 
          ...
          summand <- conv_dbetaV*(bla$rhs+ bla$dampDpD * conv_dbetaV) 
      ...
      if (gainratio<0) { ## restore everything
        ...
        betaV <- old_betaV ## old_betaV should be in valid space  hence no need to check it
        if (pforpv>0) {
          beta_eta <- betaV[seq_len(pforpv)]
          names(beta_eta) <- colnames(X.pv)
        ...
        #checkv_h <- attr(u_h,"v_h") ## verif is v_h was corrected 
        #if (!is.null(checkv_h)) v_h <- checkv_h
        #if (!is.null(attr(u_h,"v_h"))) v_h <- attr(u_h,"v_h")
        eta <- as.matrix(off + X.pv %*% beta_eta + ZAL %id*id% v_h) ## updated at each inner iteration
