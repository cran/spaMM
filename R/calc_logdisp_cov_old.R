.dwdloglam_fn_obsolete <- function(lmatrix,object,dvdloglamMat) {
  corr.model <- attr(lmatrix,"corr.model")
  if (identical(corr.model,"adjacency") || identical(corr.model,"ar1")) {
...
.loglamInfo_fn_obsolete <- function(randit,strucList,lambda.object,corrPars,asDmLR_invV,ZAL) {
  RES <- list()
  lmatrix <- strucList[[randit]]
  corr.model <- attr(lmatrix,"corr.model")
  if (identical(corr.model,"adjacency") || identical(corr.model,"ar1")) { 
    glmit <- lambda.object$rand_to_glm_map[randit]
    RES$lambda <- with(lambda.object,linkinvS[[glmit]](coefficients_lambdaS[[randit]][1]))
    ...
    RES$lhs_invV.dVdlam <- sweep( ZALd,1L,asDmLR_invV$invD,`*`) - lhs_invV.dVdlam
    RES$rhs_invV.dVdlam <- t(ZALd)
  } else if (identical(corr.model,"random-coef")) {
    stop("code missing for random-coefficient models.")
  } else { ## standard lamScal model
    RES$lambda <- exp(unlist(lambda.object$coefficients_lambdaS))
    lhs_invV.dVdlam <- asDmLR_invV$n_x_r %*% (asDmLR_invV$r_x_n %*% ZAL)  
    RES$lhs_invV.dVdlam <- sweep( ZAL,1L,asDmLR_invV$invD,`*`) - lhs_invV.dVdlam
    RES$rhs_invV.dVdlam <- t(ZAL)
  }
  # lambda^2 *sum(invV.dVdlam* t(invV.dVdlam)) :
  RES$loglamInfo <- with(RES,lambda^2 * .traceAB(lhs_invV.dVdlam, rhs_invV.dVdlam,
