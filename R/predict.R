`%id*id%` <- function(A,B) {
  if (inherits(A,"identityMatrix")) return(B)
  if (inherits(B,"identityMatrix")) return(A)
  return(A %*% B)
}
...
calcPredVar <- function(phi,lambda,Coldnew,Cnewnew,oldX.pv,X.pv,ZA,newZA,beta_cov,Sig) {
  if (is.list(newZA)) {
    ...
    sumLambdaZCZtfn <- function(lambda_short,leftZA,rightZA,Clist) {
      varMatList <- lapply(seq_len(nrand), function(it) {
        ## still assuming a single lambda for each u.range (otherwise needs true cov mat instead of Clist[[it]]):
        lambda_short[it] * leftZA[[it]] %id*id% Clist[[it]] %id*id% t(rightZA[[it]]) ## %id*id% further keeps col names of C if tZA==I
      })
      Reduce("+", varMatList) ## matrix sum, https://stat.ethz.ch/pipermail/r-help/2008-June/163817.html; garde noms première mat...
    }
    ...
    invSig.lamZCZt_on <- solve(Sig,lamZCZt_on) ## obs*obs.obs*new = obs*new
    if (inherits(newZA,"identityMatrix")) {
...
calcResidVar <- function(object,newX=NULL) {
  phi.object <- object$phi.object
  if ( ! is.null(phi.object$phi.Fix)) {
    if (length(phi.object$phi.Fix)==1) {
      residVar <- rep(phi.object$phi.Fix,nrow(newX))
    } else if (is.null(newX)) {
      residVar <- rep(phi.object$phi.Fix)          
    } else {
      stop("Unable to compute 'residVar' given 'newX' argument and given vector of phi values in fit object.")
    }
  } else {
    ...
    residVar <- predict.HLfit(pseudoObject, newX=newX,binding=FALSE,
...
## FR->FR comparer à makeCheckGeoMatrices
calcNewCorrs <- function(object,locdata,predVar,spatial.model) {
  ...
  if (predVar)  Cnewnew <- proxy::dist(newgeoScal,newgeoScal) ## dist matrix and then call to correl fn
...
predict.HLfit <- function(object,newX = NULL,coeffs=NULL,re.form = NULL,
  ...
  if (is.null(variances$fixef)) variances$fixef <- variances$sum
  if (is.null(variances$ranef)) variances$ranef <- variances$sum
  ...
  if (predVar == "Cov") {
    variances$ranef <- TRUE; variances$cov <- TRUE
  } else if ( ! is.null(predVar) && predVar) variances$ranef <- TRUE
  variances$sum <- length(which(unlist(variances[c("fixef","ranef","resid")])))>1
  ##
  locform <- attr(object$predictor,"oriFormula")
  if (length(locform)==2L) locform <- as.formula(paste('"LHS"',paste(locform,collapse=" "))) ## same syntax as in summary...
  spatial.terms <- findSpatial(locform) ## syntax requires LHS
  ...
  ## it is important that newX remains NULL if it was so initially because it is tested below. Hence copy in locdata
  if (is.null(newX)) {
    locdata <- object$data[,allvars,drop=FALSE]
  } else {
    locdata <- newX ## FR->FR [,allvars,drop=FALSE] ?
    if (any(is.na(locdata))) {
      stop("NA's in required variables from 'newX'. Prediction not possible.")
    }
  }
  #   if (is.null(newX)) {
  #     locdata <- object$data
  ...
  if ( ( ! is.null(newX) ) || variances$ranef || noReForm(re.form)) {
  ...
  if (variances$ranef) { ## then we need X.pv; if newX present then we also compute etaFix 
    X.pv <- allFrames$X  
  } else X.pv <- NULL
  ...
  if ( ! is.null(spatial.model) &&
         ( ! is.null(newX) || variances$ranef) ) { ## all cases where we need distance matrices
    blob <- calcNewCorrs(object=object,locdata=locdata,predVar=variances$ranef,spatial.model=spatial.model)
    uuCnewold <- blob$uuCnewold
    attr(uuCnewold,"ranefs") <- attr(oldLMatrix,"ranefs")  
    if (variances$ranef) Cnewnew <- blob$Cnewnew
  } 
  ...
  if (noReForm(re.form)) {
    ...
    if ( nrand==0 ) {
      ...
      newZAlist <- lapply(seq_len(length(tempZAlist)),function(it) {
        newZA <- tempZAlist[[it]] ##  
        if (is.identity(newZA)) class(newZA) <- c("identityMatrix",class(newZA))
        newZA
      })
      ...
      if ( is.null(spatial.model)) {
        ...
        ZALlist <- compute.ZALlist(CMatrix=uuCnewold,ZAlist=newZAlist,Groupings=FL$Groupings)
      }  
      if (nrand>1) {
  ...
  if(variances$ranef) {
    ...
    if (nrand>0) newnewClist <- lapply(seq_len(length(newZAlist)),function(it) {
      if (length(spatialOne)>0 && it == spatialOne) { ## avoids running the next algo which is slow on large matrices
        ...
        #         } else {
        #           ## only the diagonal is correct, but it is sufficient  ## FR->FR but is this confusing hack worthwhile?  
        #           Cnn <- diag(rep(1,nrow(newgeoScal))) ## directly "corr" of u_h with itself at zero distance
        #           attr(Cnn,"identityMatrix") <- TRUE
        #         }
      } else { 
        Cnn <- diag(ncol(newZAlist[[it]]))
        class(Cnn) <- c("identityMatrix",class(Cnn))
      }
      return(Cnn)
    })
    ...
    if (nrand>0L) oldnewClist <- lapply(seq_len(length(object$ZAlist)),function(it) {
      if (length(spatialOne)>0L && it == spatialOne) { ## avoids running the next algo which is slow on large matrices
        ...
        if (identical(oldlevels,newlevels)) class(oldnewC) <- c("identityMatrix",class(oldnewC))
      }
      return(oldnewC)
    })
    if (nrand>0L) {
    ...
    } else attr(resu,"predVar") <- diag(predVarMat)
    sumVar <- attr(resu,"predVar") ## vector or matrix
  } else sumVar <- 0  
  if (variances$resid) {
    if (object$family$family %in% c("poisson","binomial")) {
      attr(resu,"residVar") <- object$family$variance(fv)
    } else attr(resu,"residVar") <- calcResidVar(object,newX=locdata) 
