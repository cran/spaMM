...
function(dev.res,lev,X,offset=NULL) {
  ...
  weight <- (1-lev)/2 
  #etastart <- log(resp)   ## glm needs a bit help...
  ...
  resglm_wrap <- tryCatch.W.E(glm(resp~X-1,family=GammaForDispGammaGLM(link=log),weights = weight,etastart=etastart))
  resglm <- resglm_wrap$value
  attr(resglm,"warning") <- resglm_wrap$warning$message ## may be NULL
  ## on aimerait pourvoir appeler 
  ## HLfit(resp~X-1,family=GammaForDispGammaGLM(link=log),weights = weight)
  ## mais HLfit -> dispGammaGLM -> HLfit -> .... loop
  return(resglm)
}
