...
.get_inits_by_glm <- function(processed, family=processed$family, reset=FALSE) {
  if (reset || is.null(processed$envir$inits_by_glm)) { 
    ...
    if (family$family=="gaussian" && family$link=="identity") {
      ...
      resu$lambda <- resu$phi_est <- sum(dev)/resglm$df.residual # /lam_fac
    } else { ## GLM
      #
...
.preprocess <- function(control.HLfit, ranFix=NULL, HLmethod, 
  ...
  #
  ## control of maxit.mean in inner loop i.e. in .solve_IRLS_as_...(), dependent on y (Leucadendron_hard.R was revelatory)
