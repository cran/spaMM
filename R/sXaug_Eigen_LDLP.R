# 'constructor' for sXaug_EigenSparse_LDLP_scaled object
# from Xaug which already has a *scaled* ZAL 
def_sXaug_EigenSparse_LDLP_scaled <- function(Xaug,weight_X,w.ranef,H_global_scale) {
  ...
  resu <- structure(Xaug,
                    sXaug_EigenSparse_LDLP_scaled=create_sXaug_EigenSparse_LDLP_scaled(),
                    get_from="get_from_MME.sXaug_EigenSparse_LDLP_scaled",
                    w.ranef=w.ranef,
                    n_u_h=n_u_h,
...
create_sXaug_EigenSparse_LDLP_scaled <- function() {
  ...
  locfn <- function(sXaug,which="",szAug=NULL,B=NULL) {
    if (is.null(BLOB$D_scaled)) {
      BLOB <<- lmwith_sparse_LDLp(sXaug, szAug, returntQ=FALSE, returnR=TRUE,pivot=TRUE) 
    ...
    if ( ! is.null(szAug)) {
      if (is.null(BLOB$t_Q_scaled)) {
        ...
        return(Matrix::solve(BLOB$U_scaled, 
                     (BLOB$t_Q_scaled %*% szAug)/sqrt(BLOB$D_scaled))[BLOB$perm,,drop=FALSE]) 
      }
    }
    # ELSE
    if (which %in% c("absD_R_v","hatval_Z") ## attention absD_R_v est specif a LDL; sinon c'est absdiag_R_v
        || (which=="solve_d2hdv2" && is.null(BLOB$inv_d2hdv2) && !is.null(B))) {
      if (is.null(BLOB$absD_R_v)) {
        seq_n_u_h <- seq_len(attr(sXaug,"n_u_h"))
        XtX_v <- BLOB$XtX[seq_n_u_h, seq_n_u_h] ## no permutation ! 
        Rblob <-  sparse_LDLp_from_XtX(XtX_v, pivot=TRUE)
        BLOB$U_R_v <<- Rblob$U_scaled
        BLOB$sortPerm_R_v <<- Rblob$sortPerm+1L
        BLOB$perm_R_v <<- Rblob$perm+1L
        BLOB$absD_R_v <<- abs(Rblob$D_scaled)
      }
    }
    if ( is.null(BLOB$d2hdv2) && 
      ...
      w_R_R_v <- Matrix_times_Dvec(BLOB$U_R_v,sqrt(w.ranef)[BLOB$perm_R_v])
      w_R_R_v  <- Diagonal(x=sqrt(BLOB$absD_R_v)) %*% w_R_R_v
    ...
    } else if (which=="absD_R_v") { 
      return(BLOB$absD_R_v)
  ...
  environment(locfn) <- list2env(list(BLOB=list()),
                                 parent=environment(def_sXaug_EigenSparse_LDLP_scaled))
  return(locfn)
} 

get_from_MME.sXaug_EigenSparse_LDLP_scaled <- function(sXaug,which="",szAug=NULL,B=NULL,
                                        damping, LMrhs, ...) {
  resu <- switch(which,
                 "logdet_R_scaled_v" = {
                   absD_R_v <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="absD_R_v")
                   sum(log(absD_R_v))/2
                 },
                 "logdet_R_scaled_b_v" = {
                   D_scaled <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="D_scaled")
                   sum(log(abs(D_scaled)))/2
                 },
                 "logdet_sqrt_d2hdv2" = {
                   w.ranef <- attr(sXaug,"w.ranef")
                   absD_R_v <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="absD_R_v")
                   sum(log(w.ranef * absD_R_v))/2
                 },
                 "LevMar_step" = {
                   ## FR->FR probably not the most elegant implementation 
                   R_scaled <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="R_scaled")
                   sortPerm <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="sortPerm")
                   diag_pRtRp <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="diag_pRtRp")
                 ...
                 "logdet_sqrt_d2hdbeta2" = {
                   ...
                   D_scaled <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="D_scaled")
                   absD_R_v <- attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which="absD_R_v")
                   sum(log(abs(D_scaled)))/2 - sum(log(absD_R_v))/2 - attr(sXaug,"pforpv")*log(H_global_scale)/2 
                 },
                 ## all other cases:
                 attr(sXaug,"sXaug_EigenSparse_LDLP_scaled")(sXaug,which=which,szAug=szAug,B=B)
  )
  return(resu)
}
