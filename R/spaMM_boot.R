...
.dopar <- local({
  ...
  function(newresp, fn, nb_cores=NULL, 
    ...
    if (nb_cores>1L) {
      ...
      if (has_doSNOW) {
        # loading (?) the namespace of 'snow' changes the global RNG state! so we save and restore it 
        R.seed <- get(".Random.seed", envir = .GlobalEnv)
        rdS_fn <- get("registerDoSNOW", asNamespace("doSNOW"))
        do.call(rdS_fn,list(cl=cl)) 
        ...
        if (is.null(foreach_args$.errorhandling))  foreach_args$.errorhandling <- "remove" ## use "pass" to see problems
        if (is.null(foreach_args$.combine))  foreach_args$.combine <- "cbind" ## use "pass" to see problems
        ...
        if (TRUE) {
          ...
          bootreps <- try(foreach::`%dopar%`(foreach_blob, do.call(fn, c(list(y = newresp[, i]), fn_dots)))) 
        } else {
          #bootreps <- try(foreach::`%dopar%`(foreach_blob, fn(y = newresp[, i], ...)))
        }
